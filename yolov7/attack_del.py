...
def fgsm_attack(model, x, y, epsilon, layers, filepath, img0_shape, outdir, txts_path, rounds):
    ...
    for iteration in range(rounds):
        ...
        # loss = loss_function(xpoint, target)+loss_function(ypoint, target)
        ...
        # loss = loss_function(ELAN24_target_features, ELAN24_features)/2+loss_function(ELAN37_target_features, ELAN37_features)/2
        # loss = loss_function(ELAN37_target_features, ELAN37_features) / 2 + loss_function(ELAN50_target_features,ELAN50_features) / 2
        print(loss)
...
def do_fgsm_attack(model, indir, outdir, epsilon, rounds, layers, doc):
    ...
    for filepath in images:
        ...
        target_im_path = 'inference/'+doc+'/del_target/' + filename + '_comp.jpg'
...
if __name__ == '__main__':
    ...
    iname = 'origin' if args.i is None else str(args.i)
    oname = 'output_del' if args.o is None else str(args.o)
    epsilons = [0.01, 0.005, 0.001, 0.0008, 0.0005, 0.0003] if args.eps is None else list(args.eps)
    rounds   = [  30,   50,   200,    300,     400,    600] if args.rds is None else list(args.rds)
